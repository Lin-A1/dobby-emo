#!/bin/bash
# 以 E2 微醺奶油定稿形象为基准，批量生成表情
KEY="${APINEBULA_KEY:?请先 export APINEBULA_KEY=你的密钥}"
REF="G:/Code/Agents/Custom/DobbyEmo/assets/base/cand-e2-subtle.png"
OUT="G:/Code/Agents/Custom/DobbyEmo/assets/sw-white/raw"
mkdir -p "$OUT"

BASE='Based on the character in the reference image (a cute CREAM VANILLA 3D mascot with a warm white oval face mask, muted dusty-blush inner ears, brown eyes, lavender laptop, sitting): change ONLY the facial expression. Keep everything else EXACTLY the same: cream vanilla body color, subtle muted inner ears, sitting pose, lavender laptop, floating purple code symbols, 3D soft vinyl toy style. Keep the background FULLY TRANSPARENT (alpha), clean edges without halo. The expression:'
...
"11|eyes become two upward-curved happy arcs (like ^ ^) and a big joyful open laughing mouth with soft pink tongue"
...
"32|both eyes drawn as two dark brown X shapes, dizzy stunned error expression with a flat stunned mouth"
